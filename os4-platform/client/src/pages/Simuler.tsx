...
type Onglet = "trading" | "banque" | "ecom" | "stress" | "marche";
...
function StressLabPanel() {
  const [results, setResults] = useState<Record<string, { block: number; hold: number; allow: number; runs: number }>>({});
  const [running, setRunning] = useState<string | null>(null);
  const [filter, setFilter] = useState<string>("TOUS");

  const runScenario = useCallback((scenario: StressScenario, runs = 100) => {
    ...
    setTimeout(() => {
      ...
      setResults(prev => ({ ...prev, [scenario.id]: { block, hold, allow, runs } }));
      setRunning(null);
    }, 400);
  }, []);
  ...
  const runAll = useCallback(() => {
    let i = 0;
    const next = () => {
      if (i >= STRESS_SCENARIOS.length) return;
      const s = STRESS_SCENARIOS[i++];
      setRunning(s.id);
      setTimeout(() => {
        let block = 0, hold = 0, allow = 0;
        for (let j = 0; j < 100; j++) {
          const rng = seededRng(j * 137 + 42);
          const noise = (rng() - 0.5) * 0.15;
          const vol = Math.min(1, Math.max(0, s.params.volatility + noise));
          const coh = Math.min(1, Math.max(0, s.params.coherence + noise * 0.5));
          if (vol > 0.45 || coh < 0.20) block++;
          else if (vol > 0.30 || coh < 0.45) hold++;
          else allow++;
        }
        setResults(prev => ({ ...prev, [s.id]: { block, hold, allow, runs: 100 } }));
        setRunning(null);
        setTimeout(next, 100);
      }, 300);
    };
